...
class MultiAppMuteGUI:
    ...
    def _add_app_row(self, app_name: str):
        ...
        def on_captured(combo, a=app_name, c=capture, bs=btn_save, bm=btn_mute):
            bs.config(text="保存", bg=C_BTN_SAVE, activebackground="#2563EB")
        capture.on_captured = on_captured
    ...
    def _check_update(self):
        ...
        try:
            ...
            if r.status_code != 200:
                print(f"[update] version.txt の取得に失敗: HTTP {r.status_code}")
                return
            remote = r.text.strip()
            if self._parse_version(remote) <= self._parse_version(CURRENT_VERSION):
                print(f"[update] 最新バージョンです ({CURRENT_VERSION})")
                return

            current_exe = sys.argv[0]
            if not current_exe.endswith(".exe"):
                print("[update] .py から実行中のためスキップ")
                return

            print(f"[update] 新バージョン {remote} を取得中...")
            exe_r  = requests.get(EXE_URL,    timeout=60)  
            hash_r = requests.get(SHA256_URL, timeout=5)
            if exe_r.status_code != 200 or hash_r.status_code != 200:
                print(f"[update] ダウンロード失敗: exe={exe_r.status_code}, hash={hash_r.status_code}")
                return
